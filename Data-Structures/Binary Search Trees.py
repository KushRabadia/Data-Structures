...
class BST:
    ...
    def _delete_node_left(self, parent):
        ...
        Use: repl_node = self._delete_node_left(node)
        -------------------------------------------------------
        Parameters:
    ...
    def is_identical_aux(self, node, node2):
        if node is None and node2 is None:
            identical = True
        elif node is not None and node2 is not None and node._value == node2._value and node._height == node2._height:
            identical = self.is_identical_aux(node._left, node2._left) and self.is_identical_aux(node._right,
                                                                                                 node2._right)
        else:
            identical = False
        return identical
    ...
    def parent_r(self, key):
        ...
        node = self._root
        parent = None
        return self.parent_r_aux(node, key, parent)
    ...
    def leaf_count(self):
        ...
        ---------------------------------------------------------
        """
        number = self._leaf_count_aux(self._root)
        return number
    ...
    def two_child_count(self):
        ...
        count = self.two_child_count_aux(self._root)
        return count

    def two_child_count_aux(self, node):
        if node is None:
            count = 0
        elif node._left is not None and node._right is not None:
            count = 1 + self.two_child_count_aux(node._left) + self.two_child_count_aux(node._right)
        else:
            count = self.two_child_count_aux(node._left) + self.two_child_count_aux(node._right)
    ...
    def one_child_count(self):
        ...
        count = self.one_child_count_aux(self._root)
        return count
    ...
    def one_child_count_aux(self, node):
        if node is None:
            ...
            count = self.one_child_count_aux(node._left) + self.one_child_count_aux(node._right)
        elif node._left is None and node._right is None:
            count = self.one_child_count_aux(node._left) + self.one_child_count_aux(node._right)
        else:
            count = 1 + self.one_child_count_aux(node._left) + self.one_child_count_aux(node._right)
