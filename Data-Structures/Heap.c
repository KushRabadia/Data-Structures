...
Heap* create_heap(char *type) {
	...
	h->root = NULL;
	h->size = 0;
	if (strcmp(type, "max") != 0 && strcmp(type, "min")) {
...
Data* peek_heap(Heap *h) {
	assert(h);
	if (is_empty_heap(h)) {
		printf("Error(peak_heap): heap is empty\n");
		return NULL;
	}
...
Node* find_node_heap(Heap *h, Data *d) {
	Node *n = find_node_heap_aux(h->root, d);
	return n;
}
...
Node* find_node_heap_aux(Node *n, Data *d) {
	...
	if (compare_data(d, n->data) == 0) {
		...
		if (n->left != NULL)
			m = find_node_heap_aux(n->left, d);
		if (m != NULL && compare_data(m->data, d) == 0)
			return m;
		if (n->right != NULL)
			m = find_node_heap_aux(n->right, d);
...
int contains_heap(Heap *h, Data *d) {
	...
	int i = contains_heap_aux(h, h->root, d);
	return i;
}
...
int find_node_level_heap_aux(Node *start, Node *node, int level) {
	...
	if (compare_data(start->data, node->data) == 0) {
		...
		if (start->left != NULL)
			downlevel = find_node_level_heap_aux(start->left, node, level + 1);
		if (downlevel != 0)
			return downlevel;
		if (start->right != NULL)
...
int find_node_level_heap(Heap *h, Node *node) {
	int i = find_node_level_heap_aux(h->root, node, 1);
	return i;
}
...
int is_full_heap_aux(Node *root) {
	...
	if (root->left != NULL && root->right != NULL) {
		i = is_full_heap_aux(root->left);
		if (i == 0)
			return 0;
		i = is_full_heap_aux(root->right);
 ...
 * --------------------------------------------------
 */
Data* find_max_heap_aux(Node *root, Data *max) {
	if (root == NULL)
		return NULL;
	if (compare_data(max, root->data) == 2)
		max = root->data;
	if (root->left != NULL)
		max = find_max_heap_aux(root->left, max);
	if (max != NULL && compare_data(max, root->data) == 2)
		max = root->data;
	if (root->right != NULL)
		max = find_max_heap_aux(root->right, max);
...
Data* find_max_heap(Heap *h) {
	...
	else if (strcmp(h->type, "max") == 0)
		return copy_data(h->root->data);
	else {
		return copy_data(find_max_heap_aux(h->root, h->root->data));
	}
 ...
 * --------------------------------------------------
 */
Data* find_min_heap_aux(Node *root, Data *min) {
	if (root == NULL)
		return NULL;
	if (compare_data(min, root->data) == 1)
		min = root->data;
	if (root->left != NULL)
		min = find_min_heap_aux(root->left, min);
	if (min != NULL && compare_data(min, root->data) == 1)
		min = root->data;
	if (root->right != NULL)
		min = find_min_heap_aux(root->right, min);
...
Data* find_min_heap(Heap *h) {
	if (is_empty_heap(h))
		...
		return copy_data(find_min_heap_aux(h->root, h->root->data));
