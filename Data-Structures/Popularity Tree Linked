...
class Popularity_Tree:
    ...
    def _retrieve_aux(self, node, key):
        ...
        if node is None:
            ...
            if key < node._value:
                node._left,value = self._retrieve_aux(node._left, key)
            elif key > node._value:
                node._right, value = self._retrieve_aux(node._right, key)
            else:
                value = deepcopy(node._value)
                node._rcount += 1
        
        if value != None and node._value != key:
            if node._right._rcount > node._rcount:
                self._rotate_left(node)
            elif node._left._rcount < node._rcount:
    ...
    def _rotate_left(self, parent):
        ...
        r_child = parent._right
        
        l_r_child = r_child._left
        parent._right = l_r_child
        r_child._left = parent
                
        
        parent._update_height()
        r_child._update_height()
    ...
    def _rotate_right(self, parent):
        ...
        -------------------------------------------------------
        """
        l_child = parent._left
        parent._right = None
        
        l_r_child = l_child._right
        l_child._right = parent
        
        parent._left = l_r_child
        
        parent._update_height()
        l_child._update_height()
