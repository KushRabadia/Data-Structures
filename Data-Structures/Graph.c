...
int add_vertex_graph(Graph *g, Vertex *v) {
	...
	//assign the new vertex a sequential number & increment vertex count
...
int has_edge_graph(Graph *g, Edge *e) {
	...
	//Graph does not contains directed or weighted edges
...
int add_edge_graph(Graph *g, Edge *e) {
	...
	//check for compatibility in terms of weight an d directed
	if (e->directed || e->weight) {
	...
	g->edge_count++; // increment number of edges
	return True;
}
...
int remove_vertex_graph(Graph *g, Vertex *v) {
	...
	//if edge does not exist
	if (is_empty_graph(g) || !has_vertex_graph(g, v)) {
